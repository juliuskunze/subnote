 ...
 * <p>
 * <p>
 * The result will be received in the onActivityResult callback of the Activity or Fragment supplied to {@link DropboxChooser#launch}.
 * </p>
 */
public class DropboxChooser {
    ...
    public DropboxChooser(String appKey) {
     ...
     * file.
     */
    public DropboxChooser forResultType(ResultType resultType) {
        if (resultType == null) {
            throw new IllegalArgumentException("An app key must be supplied.");
        }
     ...
     */
    public DropboxChooser pretendNotAvailable() {
    ...
    public void launch(Activity act, int requestCode) throws ActivityNotFoundException {
        ...
        ActivityLike thing = new ActivityLike() {
            @Override
            public Activity getActivityIfHas() {
                return mAct;
            }
    ...
    private void doAppStoreFallback(ActivityLike thing, int requestCode) throws ActivityNotFoundException {
        Toast.makeText(thing.getActivityIfHas(), "Please install Dropbox.", Toast.LENGTH_SHORT).show();
    }
    ...
    public static class Result {
         ...
         *               If the request code doesn't match the one provided to {@link DropboxChooser#launch(Activity, int)} or if the result code
